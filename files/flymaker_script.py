...
def generate_all_strings(tob_list, ke_list, direction_list, source_position_list):
    particle_params = []
    
    particle_params.append(set_tob(tob_list))
    particle_params.append(set_KE(ke_list))
    particle_params.append(set_direction(direction_list))
    particle_params.append(set_source_position(source_position_list))
    
    return particle_params


def write_fly_file(workbench_name, mass, charge, particle_params):
# The name of the fly file must be the same as the workbench in order for SIMION to see it
  fly_name = f'{workbench_name}.fly2'
  with open(fly_name, 'w') as file:
...
particles {
  ...
  standard_beam {
     n = 1, \n"""
     + particle_params[0] # TOB
     + """\n     mass = """+str(mass)+""",
     charge = """+str(charge)+""",\n"""
     + particle_params[1] + """\n""" # ke
     + particle_params[2] + """\n""" # Direction: azm and el
     + """     cwf = 1 
     color = 1 \n"""
     + particle_params[3] +"""\n""" # Source position
     + """  }
