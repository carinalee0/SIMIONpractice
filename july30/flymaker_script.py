import numpy as np
# Creates and writes the fly file for simulations.
...
def set_azm(input_list):
    ...
    if 'arithmetic sequence' in input_list[0]:
        azm_str = '     az = arithmetic_sequence { \n        first = ' + str(input_list[1]) + ', \n        step = ' + str(input_list[2]) + ', \n        n = ' + str(input_list[3]) + ' \n     },'
    
    if 'gaussian' in input_list[0]:
        azm_str = '     az = gaussian_distribution { \n        mean = ' + str(input_list[1]) + ', \n       stdev = ' + str(input_list[2]) + ', \n        fwhm = ' + str(input_list[3]) + ' \n     },'
...
def set_tob (input_list):
    ...
    if 'arithmetic sequence' in input_list[0]:
        tob_str = '     tob = arithmetic_sequence { \n        first = ' + str(input_list[1]) + ', \n        step = ' + str(input_list[2]) + ' \n     },'
...
# Creates a string for each new group of particles / anytime a parameter is changed
...
# Generate a param list. Each param list contains definitions for one group of particles, multiple groups of particles will have multiple param lists. Need to call this.
...
def write_flyfile (file_name, param_list):
    ...
    with open(file_name, 'w') as file:
        file.write("""
particles {
    coordinates = 0""" + group_str + """
    }""")

#write_flyfile("C:\\Users\\carina\\SULI2024\\SIMION_files_2024\\front_lens.fly2", p_list)

#if __name__ == '__main__':
#    get_group_string(['',0],1,-1,['gaussian',300,5],[['',0],['',0]],['single vector', 20,0,0])